...
pri.serializeURI();if(angular.isDefined(ns.cache[model][id]))return true;return false},serializeURI:function(){var params=$location.search(),serial=[],keys=pri._keys(params).sort(),i=0;serial.push('"'+$location.path()+'"');for(i=0;i<keys.length;i++)serial.push('"'+keys[i]+"="+params[keys[i]]+'"');return serial.join(",")},getObjValue:function(obj,p){if(p=="")return obj;var val=obj,path=p.split("."),i=0,node="",index=0,patt=/\[(.*)\]$/;for(i=0;i<path.length;i++){node=path[i];if(node.indexOf("[")>-1){index=
parseInt(node.match(patt)[1]);node=node.replace(patt,"");if(node=="")val=val[index];else val=val[node][index]}else if(node!=""&&angular.isDefined(val[node]))val=val[node]}return val},setObjValue:function(obj,p,value){if(p==""){obj=value;return}var path=p.split("."),last=path.length-1,i=0,node="",index=0,patt=/\[(.*)\]$/;for(i=0;i<last;i++){node=path[i];if(node.indexOf("[")>-1){index=parseInt(node.match(patt)[1]);node=node.replace(patt,"");if(node=="")obj=obj[index];else{if(node in obj===false){obj[node]=
[];obj[node][index]={}}obj=obj[node][index]}}else if(node!=""){if(node in obj===false)obj[node]={};obj=obj[node]}}obj[path[last]]=value},_keys:Object.keys||function(obj){if(obj!==Object(obj))throw new TypeError("Invalid object");var keys=[];for(var key in obj)if(hasOwnProperty.call(obj,key))keys[keys.length]=key;return keys}};pri.prepopulateData();var service={};service.addModel=angular.bind(service,pub.addModel,null);service.scopeModel=angular.bind(service,pub.scopeModel,null);service.set=angular.bind(service,
pub.set,null);service.update=angular.bind(service,pub.update,null);service.value=angular.bind(service,pub.value,null);service.append=angular.bind(service,pub.append,null);service.clearCache=angular.bind(service,pub.clearCache,null);service.clear=angular.bind(service,pub.clear,null);service.startIntervalUpdate=angular.bind(service,pub.startIntervalUpdate,null);service.stopIntervalUpdate=angular.bind(service,pub.stopIntervalUpdate,null);service.save=angular.bind(service,pub.save,null);service.remove=
angular.bind(service,pub.remove,null);return service}]});